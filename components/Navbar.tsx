          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-center space-x-8">
              ...
              <div className="relative group ml-4">
                <button className="flex items-center space-x-1 hover:text-gray-300 border border-white/20 px-3 py-1 rounded-full bg-white/5">
                  <Globe className="h-3.5 w-3.5" />
                  <span className="uppercase text-xs font-bold">{lang}</span>
                </button>
                <div className="absolute right-0 mt-2 w-32 bg-white rounded-lg shadow-xl py-1 hidden group-hover:block ring-1 ring-black ring-opacity-5 overflow-hidden">
                  {languages.map((l) => (
                    <button
                      key={l.code}
                      onClick={() => setLang(l.code)}
                      className={`block w-full text-left px-4 py-2 text-sm text-gray-800 hover:bg-gray-100 ${lang === l.code ? 'font-bold bg-gray-50 text-brand-red' : ''}`}
                    >
                      {l.label}
                    </button>
                  ))}
